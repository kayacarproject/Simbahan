...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(auth)` | `/(auth)/join-church` | `/(auth)/login` | `/(auth)/register` | `/(public)` | `/(public)/` | `/(tabs)` | `/(tabs)/` | `/(tabs)/announcements` | `/(tabs)/calendar` | `/(tabs)/more` | `/(tabs)/parish` | `/(tabs)/schedule` | `/_sitemap` | `/announcements` | `/calendar` | `/community` | `/country-select` | `/donations` | `/donations/history` | `/events` | `/family` | `/holy-week` | `/home` | `/join-church` | `/language-select` | `/login` | `/mass-schedule` | `/more` | `/notifications` | `/novenas` | `/novenas/rosary` | `/parish` | `/parish-info` | `/profile` | `/profile/edit` | `/readings/today` | `/register` | `/sacraments` | `/sacraments/request` | `/schedule` | `/settings` | `/simbang-gabi` | `/welcome`;
